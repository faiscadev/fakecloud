...
description = "fakecloud is a free, open-source local AWS emulator: 101 services, 6,453 operations, 216,074/216,074 Smithy variants pass (true 100% conformance), 6 test-assertion SDKs. No account, no token, no paid tier. Drop-in replacement for LocalStack Community."
template = "page.html"
aliases = [
...
| Conformance methodology | Smithy-model-validated, 216,074/216,074 variants pass | Not published |
...
For every operation exposed by AWS's Smithy model, fakecloud accepts every documented input shape and returns the documented output shape, with every field AWS returns. Validated on every commit against 216,074 generated test variants — every single one passes, no flake margin and no skipped services — plus the upstream `hashicorp/terraform-provider-aws` `TestAcc*` suites. This applies to every service listed above.
