...
description = "fakecloud is a free, open-source local AWS emulator: 49 services, 3,858 operations, 128,643/128,643 Smithy variants pass (true 100% conformance), 6 test-assertion SDKs. No account, no token, no paid tier. Drop-in replacement for LocalStack Community."
template = "page.html"
aliases = [
...
- **3,858 API operations. True 100% conformance** across every implemented service — 128,643/128,643 Smithy-model-generated test variants pass on every commit.
...
| Conformance methodology | Smithy-model-validated, 128,643/128,643 variants pass | Not published |
...
For every operation exposed by AWS's Smithy model, fakecloud accepts every documented input shape and returns the documented output shape, with every field AWS returns. Validated on every commit against 128,643 generated test variants — every single one passes, no flake margin and no skipped services — plus the upstream `hashicorp/terraform-provider-aws` `TestAcc*` suites. This applies to every service listed above.
