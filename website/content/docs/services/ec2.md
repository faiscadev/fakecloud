...
- **Security-group / NACL packet filtering needs host privileges.** Real enforcement (above) requires `CAP_NET_ADMIN` + `nft` on Docker/Podman, or a NetworkPolicy-enforcing CNI on Kubernetes. Where neither is available the rules are stored and returned faithfully but tracked-only; L3 (per-subnet/VPC) isolation still applies. ICMP and IPv6 rules are modeled best-effort.
