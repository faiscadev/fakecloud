...
- **Interactive sessions** — `CreateSession` / `GetSession` / `ListSessions` / `DeleteSession` / `StopSession`, statements (`RunStatement` / `GetStatement` / `ListStatements` / `CancelStatement`). Statement ids auto-increment per session.
