...
> Key generation always produces a genuine key of the requested algorithm and size in every build (no substitution). Because real RSA-4096 generation can take tens of seconds, key generation runs on a background task: the CA is `CREATING` until its key is ready, then becomes `PENDING_CERTIFICATE`. The transition is persisted, so a CA whose key was still generating when the process exited resumes generation on restart.
