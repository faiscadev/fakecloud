...
| `--dynamodb-import-path` | `FAKECLOUD_DYNAMODB_IMPORT_PATH` | —          | Bulk-load a single AWS-format DynamoDB S3 export as a new table at startup. Points at one export dir (`AWSDynamoDB/<export-id>/`, holding `manifest-summary.json`). Requires `--dynamodb-import-describe-table`; conflicts with `--dynamodb-import-dir`. Additive — writes straight into the store, no `BatchWriteItem` and no effect on the recorded `ImportTable` op. `DYNAMODB_JSON` format only; startup fails if a table of that name already exists. See [DynamoDB](/docs/services/dynamodb/#importing-an-aws-export-at-startup). |
| `--dynamodb-import-describe-table` | `FAKECLOUD_DYNAMODB_DESCRIBE_TABLE` | — | Path to an `aws dynamodb describe-table` JSON dump supplying the table shape (key schema, attribute definitions, indexes, billing mode) for `--dynamodb-import-path`. Required together with `--dynamodb-import-path`; conflicts with `--dynamodb-import-dir`. |
| `--dynamodb-import-dir` | `FAKECLOUD_DYNAMODB_IMPORT_DIR` | — | Bulk-load many AWS-format DynamoDB S3 exports as new tables at startup. Points at a root directory of per-table subdirectories, each self-contained with its own `describe-table.json`. Conflicts with `--dynamodb-import-path`/`--dynamodb-import-describe-table`. Same additive/`DYNAMODB_JSON`-only rules as `--dynamodb-import-path`; startup fails if any subdirectory is missing its `describe-table.json`. See [DynamoDB](/docs/services/dynamodb/#importing-an-aws-export-at-startup). |
...
| `--dns` | `FAKECLOUD_DNS` | `false` | Run a real DNS resolver that answers `A`/`AAAA`/`CNAME`/`MX`/`TXT`/`NS`/`PTR`/`SPF`/`CAA` from the Route 53 records created in fakecloud. Point a container's resolver at fakecloud (compose `dns:` / `/etc/resolv.conf`) and created records resolve to their local targets; names in no Route 53 zone forward upstream. See [DNS resolver](/docs/guides/dns/). |
| `--dns-addr` | `FAKECLOUD_DNS_ADDR` | `0.0.0.0:53` | Address the DNS resolver binds (UDP + TCP). Binding port 53 needs root, so pass e.g. `127.0.0.1:15353` for an unprivileged run. Only used with `--dns`. |
| `--dns-upstream` | `FAKECLOUD_DNS_UPSTREAM` | first non-loopback `nameserver` in `/etc/resolv.conf`, else `8.8.8.8:53` | Upstream resolver for names in no Route 53 zone, so a container can use fakecloud as its sole resolver and still reach the outside world. Only used with `--dns`. |
