...
| `--dynamodb-import-path` | `FAKECLOUD_DYNAMODB_IMPORT_PATH` | —          | Bulk-load AWS-format DynamoDB S3 export(s) as new table(s) at startup. Two modes, picked by whether `--dynamodb-import-describe-table` is also set: set together, this is one export dir (`AWSDynamoDB/<export-id>/`, holding `manifest-summary.json`) for a single table; set alone, this is a root directory of per-table subdirectories (each self-contained with its own `describe-table.json`), imported as multiple tables. Additive — writes straight into the store, no `BatchWriteItem` and no effect on the recorded `ImportTable` op. `DYNAMODB_JSON` format only; startup fails if a table of that name already exists, or (multi-table mode) if any subdirectory is missing its `describe-table.json`. See [DynamoDB](/docs/services/dynamodb/#importing-an-aws-export-at-startup). |
| `--dynamodb-import-describe-table` | `FAKECLOUD_DYNAMODB_DESCRIBE_TABLE` | — | Path to an `aws dynamodb describe-table` JSON dump supplying the table shape (key schema, attribute definitions, indexes, billing mode) for `--dynamodb-import-path` in single-table mode. Omit it to import `--dynamodb-import-path` as a multi-table root directory instead; setting it alone (without `--dynamodb-import-path`) aborts startup. |
