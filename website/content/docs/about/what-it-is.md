...
A free, open-source local AWS emulator for integration testing and local development. For every service it implements, the goal is 100% behavioral parity with real AWS — measured by a schema-driven [conformance harness](/docs/about/conformance/) that runs 136,154 generated test variants against official AWS Smithy models on every commit, all of which pass.
...
**Not a drop-in for all of AWS.** It implements 54 services — the ones most teams actually test against. If you need EKS, Redshift, or SageMaker, fakecloud isn't the right tool (yet).
...
That's why the project prioritizes conformance and cross-service wiring over breadth. 54 services at true 100% (136,154/136,154 Smithy variants pass) is more useful than 100 services at 50%.
