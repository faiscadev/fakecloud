...
use crate::state::{CeSnapshot, SharedCeState, CE_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read ce persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse ce persistence snapshot: {0}")]
    Parse(String),
    #[error("ce persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub async fn save_snapshot(
    ...
    let snapshot = CeSnapshot {
        schema_version: CE_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write ce snapshot"),
        Err(err) => tracing::error!(%err, "ce snapshot task panicked"),
    }
}
...
mod tests {
    use super::*;
    use crate::state::{CeData, CeSnapshot};
    ...
    fn round_trip_restores_accounts() {
        let mut accounts: MultiAccountState<CeData> =
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<CeData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": CE_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
