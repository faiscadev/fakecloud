//! Snapshot save/load for AWS Resource Access Manager state.
...
use crate::state::{RamSnapshot, SharedRamState, RAM_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read ram persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse ram persistence snapshot: {0}")]
    Parse(String),
    #[error("ram persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    if snapshot.schema_version > RAM_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: RAM_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = RamSnapshot {
        schema_version: RAM_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write ram snapshot"),
        Err(err) => tracing::error!(%err, "ram snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn round_trip_restores_accounts() {
        let mut accounts: MultiAccountState<RamState> =
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<RamState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": RAM_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
