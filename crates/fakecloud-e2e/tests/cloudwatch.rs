...
async fn describe_alarms_honors_alarm_types() {
    ...
    // Default (no AlarmTypes) returns only metric alarms.
    let def = cw.describe_alarms().send().await.unwrap();
    assert!(def
        .metric_alarms()
        .iter()
        .any(|a| a.alarm_name() == Some("m1")));
    assert!(
        def.composite_alarms().is_empty(),
        "composite alarms must not be returned unless requested"
    );

    // AlarmTypes=CompositeAlarm returns the composite alarm.
    let comp = cw
        .describe_alarms()
    ...
    assert!(comp
        .composite_alarms()
        .iter()
        .any(|a| a.alarm_name() == Some("c1")));
    assert!(comp.metric_alarms().is_empty());
}
