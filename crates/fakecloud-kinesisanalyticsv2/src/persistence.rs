//! Snapshot save/load for `kinesisanalyticsv2` state.
...
use crate::state::{Ka2Snapshot, SharedKa2State, KA2_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read kinesisanalyticsv2 persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse kinesisanalyticsv2 persistence snapshot: {0}")]
    Parse(String),
    #[error("kinesisanalyticsv2 persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub async fn save_snapshot(
    ...
    let snapshot = Ka2Snapshot {
        schema_version: KA2_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write kinesisanalyticsv2 snapshot"),
        Err(err) => tracing::error!(%err, "kinesisanalyticsv2 snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<Ka2State> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": KA2_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
