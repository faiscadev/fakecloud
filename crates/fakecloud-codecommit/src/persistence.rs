...
use crate::state::{CodeCommitSnapshot, SharedCodeCommitState, CODECOMMIT_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read codecommit persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse codecommit persistence snapshot: {0}")]
    Parse(String),
    #[error("codecommit persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    let snapshot: CodeCommitSnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > CODECOMMIT_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: CODECOMMIT_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = CodeCommitSnapshot {
        schema_version: CODECOMMIT_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write codecommit snapshot"),
        Err(err) => tracing::error!(%err, "codecommit snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn state() -> SharedCodeCommitState {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<CodeCommitState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": CODECOMMIT_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
