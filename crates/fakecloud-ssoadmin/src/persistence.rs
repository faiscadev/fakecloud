...
use crate::state::{SharedSsoAdminState, SsoAdminSnapshot, SSOADMIN_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read ssoadmin persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse ssoadmin persistence snapshot: {0}")]
    Parse(String),
    #[error("ssoadmin persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    let snapshot: SsoAdminSnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > SSOADMIN_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: SSOADMIN_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = SsoAdminSnapshot {
        schema_version: SSOADMIN_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write ssoadmin snapshot"),
        Err(err) => tracing::error!(%err, "ssoadmin snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<SsoAdminData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": SSOADMIN_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
