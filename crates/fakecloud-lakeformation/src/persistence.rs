//! Snapshot save/load for AWS Lake Formation state.
...
use crate::state::{
    LakeFormationSnapshot, SharedLakeFormationState, LAKEFORMATION_SNAPSHOT_SCHEMA_VERSION,
};
...
pub enum LoadError {
    #[error("failed to read lakeformation persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse lakeformation persistence snapshot: {0}")]
    Parse(String),
    #[error(
        "lakeformation persistence schema too new: on-disk={on_disk}, max supported={supported}"
    )]
...
pub fn load_into(
    ...
    let snapshot: LakeFormationSnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > LAKEFORMATION_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: LAKEFORMATION_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = LakeFormationSnapshot {
        schema_version: LAKEFORMATION_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write lakeformation snapshot"),
        Err(err) => tracing::error!(%err, "lakeformation snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn state() -> SharedLakeFormationState {
    ...
    fn round_trip_restores_accounts() {
        let mut accounts: MultiAccountState<LakeFormationState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        accounts.get_or_create("111122223333");
        let snap = LakeFormationSnapshot {
            schema_version: LAKEFORMATION_SNAPSHOT_SCHEMA_VERSION,
            accounts,
        };
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<LakeFormationState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": LAKEFORMATION_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
