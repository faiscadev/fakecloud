//! Snapshot save/load for Glacier state.
...
use crate::state::{GlacierSnapshot, SharedGlacierState, GLACIER_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read glacier persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse glacier persistence snapshot: {0}")]
    Parse(String),
    #[error("glacier persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub async fn save_snapshot(
    ...
    let snapshot = GlacierSnapshot {
        schema_version: GLACIER_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write glacier snapshot"),
        Err(err) => tracing::error!(%err, "glacier snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<GlacierState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": GLACIER_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
