//! XML fragment renderers for DocumentDB Query-protocol responses.
//!
...
pub(crate) fn db_instance(i: &DbInstance) -> String {
    ...
    format!(
        "<DBInstanceIdentifier>{id}</DBInstanceIdentifier>\
         ...
         <Endpoint><Address>{addr}</Address><Port>{port}</Port><HostedZoneId>Z1DOCDB000000</HostedZoneId></Endpoint>\
