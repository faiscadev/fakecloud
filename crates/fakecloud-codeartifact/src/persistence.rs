...
use crate::state::{
    CodeArtifactSnapshot, SharedCodeArtifactState, CODEARTIFACT_SNAPSHOT_SCHEMA_VERSION,
};
...
pub enum LoadError {
    #[error("failed to read codeartifact persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse codeartifact persistence snapshot: {0}")]
    Parse(String),
    #[error(
        "codeartifact persistence schema too new: on-disk={on_disk}, max supported={supported}"
    )]
...
pub fn load_into(
    ...
    let snapshot: CodeArtifactSnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > CODEARTIFACT_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: CODEARTIFACT_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = CodeArtifactSnapshot {
        schema_version: CODEARTIFACT_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write codeartifact snapshot"),
        Err(err) => tracing::error!(%err, "codeartifact snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn state() -> SharedCodeArtifactState {
    ...
    fn round_trip_restores_accounts() {
        let mut accounts: MultiAccountState<CodeArtifactState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        accounts.get_or_create("111122223333");
        let snap = CodeArtifactSnapshot {
            schema_version: CODEARTIFACT_SNAPSHOT_SCHEMA_VERSION,
            accounts,
        };
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<CodeArtifactState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": CODEARTIFACT_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
