...
use crate::state::{SharedSwfState, SwfSnapshot, SWF_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read swf persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse swf persistence snapshot: {0}")]
    Parse(String),
    #[error("swf persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub async fn save_snapshot(
    ...
    let snapshot = SwfSnapshot {
        schema_version: SWF_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write swf snapshot"),
        Err(err) => tracing::error!(%err, "swf snapshot task panicked"),
    }
}
...
mod tests {
    ...
    #[test]
    fn round_trip_restores_domains() {
        let mut accounts: MultiAccountState<SwfData> =
        ...
        data.domains.insert(
            ...
            crate::state::Domain {
                ...
                arn: "arn:aws:swf:us-east-1:111122223333:/domain/orders".into(),
            },
        );
        let snap = SwfSnapshot {
            schema_version: SWF_SNAPSHOT_SCHEMA_VERSION,
            accounts,
        };
        ...
        assert_eq!(
            guard.get("111122223333").unwrap().domains["orders"].status,
            "REGISTERED"
        );
    }
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<SwfData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": SWF_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
