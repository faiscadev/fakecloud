//! Model-driven input validation for Amazon SWF.
//!
...
//! before any business logic runs. This module reproduces that: the SWF Smithy
...
//! message for the first violation. The message is surfaced as SWF's
//! request-validation fault by the caller.
...
#[cfg(test)]
pub const OPERATION_COUNT: usize = 39;
...
fn build_constraints() -> OpConstraints {
    let model: Value = serde_json::from_str(MODEL_JSON).expect("embedded swf model parses");
    let shapes = model
        .get("shapes")
...
mod tests {
    ...
    fn missing_required_member_is_rejected() {
        let err = validate_input("RegisterDomain", &json!({})).unwrap_err();
        assert!(err.contains("name") || err.contains("workflowExecutionRetentionPeriodInDays"));
    }
    ...
    fn valid_input_passes() {
        assert!(validate_input(
            "RegisterDomain",
            &json!({ "name": "orders", "workflowExecutionRetentionPeriodInDays": "30" })
        )
        .is_ok());
    }
    ...
    fn bad_enum_is_rejected() {
        let err =
            validate_input("ListDomains", &json!({ "registrationStatus": "NOPE" })).unwrap_err();
        assert!(err.contains("registrationStatus"));
    }

    #[test]
    fn all_operations_have_constraint_entries() {
        assert!(constraints().contains_key("StartWorkflowExecution"));
        assert!(constraints().contains_key("PollForDecisionTask"));
