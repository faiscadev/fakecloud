//! Model-driven input validation for Amazon Textract.
//!
...
//! any business logic runs. This module reproduces that: the Textract Smithy
...
//! the client-error shape every Textract operation declares).
...
fn build_constraints() -> OpConstraints {
    let model: Value = serde_json::from_str(MODEL_JSON).expect("embedded textract model parses");
    let shapes = model
        .get("shapes")
...
mod tests {
    ...
    fn missing_required_is_rejected() {
        // DetectDocumentText requires Document.
        let err = validate_input("DetectDocumentText", &json!({})).unwrap_err();
    ...
    #[test]
    fn adapter_id_length_is_validated() {
        // AdapterId min length is 12.
        let err = validate_input("GetAdapter", &json!({"AdapterId": "short"})).unwrap_err();
