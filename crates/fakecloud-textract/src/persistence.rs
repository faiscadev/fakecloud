//! Snapshot save/load for Amazon Textract state.
...
use crate::state::{SharedTextractState, TextractSnapshot, TEXTRACT_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read textract persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse textract persistence snapshot: {0}")]
    Parse(String),
    #[error("textract persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    let snapshot: TextractSnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > TEXTRACT_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: TEXTRACT_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = TextractSnapshot {
        schema_version: TEXTRACT_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write textract snapshot"),
        Err(err) => tracing::error!(%err, "textract snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<TextractState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": TEXTRACT_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
