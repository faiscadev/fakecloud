//! Snapshot save/load for Account Management state.
...
use crate::state::{AccountSnapshot, SharedAccountState, ACCOUNT_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read account persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse account persistence snapshot: {0}")]
    Parse(String),
    #[error("account persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    if snapshot.schema_version > ACCOUNT_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            ...
            supported: ACCOUNT_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write account snapshot"),
        Err(err) => tracing::error!(%err, "account snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn round_trip_restores_accounts() {
        let mut accounts: MultiAccountState<AccountData> =
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<AccountData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": ACCOUNT_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
