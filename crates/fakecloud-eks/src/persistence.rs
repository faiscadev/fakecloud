...
use crate::state::{EksSnapshot, SharedEksState, EKS_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read eks persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse eks persistence snapshot: {0}")]
    Parse(String),
    #[error("eks persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    if snapshot.schema_version > EKS_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: EKS_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
mod tests {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<EksState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": EKS_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
