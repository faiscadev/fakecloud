//! Snapshot save/load for Amazon EMR state.
...
use crate::state::{EmrSnapshot, SharedEmrState, EMR_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read emr persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse emr persistence snapshot: {0}")]
    Parse(String),
    #[error("emr persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub async fn save_snapshot(
    ...
    let snapshot = EmrSnapshot {
        schema_version: EMR_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write emr snapshot"),
        Err(err) => tracing::error!(%err, "emr snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<EmrState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": EMR_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
