//! Model-driven input validation for Amazon EMR.
//!
...
//! any business logic runs. This module reproduces that: the EMR Smithy model
//! is embedded and parsed once, and [`validate_input`] checks a request body's
//! top-level members against the operation's input shape, returning the message
//! for the first violation (surfaced as `InvalidRequestException`, EMR's
//! canonical client-error shape).
...
fn build_constraints() -> OpConstraints {
    let model: Value = serde_json::from_str(MODEL_JSON).expect("embedded emr model parses");
    let shapes = model
        .get("shapes")
...
mod tests {
    ...
    fn missing_required_is_rejected() {
        // RunJobFlow requires Name + Instances.
        let err = validate_input("RunJobFlow", &json!({"Instances": {}})).unwrap_err();
        assert!(err.contains("Name"), "{err}");
    }

    #[test]
    fn valid_input_passes() {
        assert!(validate_input("RunJobFlow", &json!({"Name": "c", "Instances": {}})).is_ok());
    }

    #[test]
    fn unknown_op_passes() {
        assert!(validate_input("NoSuchOp", &json!({})).is_ok());
    }

    #[test]
    fn enum_is_validated() {
        // CreateStudio.AuthMode is an enum (SSO | IAM).
        let err = validate_input(
            "CreateStudio",
            &json!({
                "Name": "s", "AuthMode": "__INVALID_ENUM_VALUE__", "VpcId": "vpc-1",
                "SubnetIds": ["subnet-1"], "ServiceRole": "role",
                "WorkspaceSecurityGroupId": "sg-1", "EngineSecurityGroupId": "sg-2",
                "DefaultS3Location": "s3://x"
            }),
        )
        .unwrap_err();
