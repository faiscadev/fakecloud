...
impl EfsService {
    ...
    fn describe_file_systems(
        ...
        let (page, next) = paginate_marker(rows, query_one(q, "Marker"), query_one(q, "MaxItems"));
        let mut out = Map::new();
    ...
    fn delete_file_system(&self, ctx: &Ctx, label: &str) -> Result<AwsResponse, AwsServiceError> {
        ...
        if has_mt {
            return Err(conflict(
                "FileSystemInUse",
                &format!("File system '{fsid}' has mount targets."),
            ));
        }
...
impl EfsService {
    ...
    fn describe_mount_targets(
        ...
        if by_fs.is_none() && by_mt.is_none() && by_ap.is_none() {
...
impl EfsService {
    ...
    fn describe_access_points(
        ...
        // AccessPoints uses NextToken pagination.
...
impl EfsService {
    ...
    fn describe_replication_configurations(
        ...
        if let Some(fsid) = &by_fs {
            let known = data
                .map(|d| d.file_systems.contains_key(fsid) || d.replications.contains_key(fsid))
                .unwrap_or(false);
            if !known {
                return Err(fs_not_found(fsid));
            }
...
impl EfsService {
    ...
    fn tag_resource(
        ...
        if rid.starts_with("fs-") {
            sync_fs_name(data, &rid);
        }
        empty(StatusCode::OK)
    }
    ...
    fn describe_account_preferences(&self, ctx: &Ctx) -> Result<AwsResponse, AwsServiceError> {
        ...
        let pref = guard
            ...
            .unwrap_or_else(|| "LONG_ID".to_string());
        ok(
            StatusCode::OK,
            json!({
...
/// Keep a file system's `Name` field and embedded `Tags` list in sync with its
/// tag map after a tag mutation.
fn sync_fs_name(data: &mut EfsData, fsid: &str) {
    let tags = data.tags.get(fsid).cloned().unwrap_or_default();
    if let Some(fs) = data.file_systems.get_mut(fsid) {
