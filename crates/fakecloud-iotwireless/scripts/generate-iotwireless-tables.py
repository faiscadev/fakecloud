#!/usr/bin/env python3
"""Regenerate crates/fakecloud-iotwireless/src/generated.rs from aws-models/iotwireless.json.

The AWS IoT Wireless control plane has 112 operations; hand-maintaining the route
...
Smithy model. Run after refreshing aws-models/iotwireless.json:

    python3 scripts/generate-iotwireless-tables.py
"""
import json, os
# scripts/ lives at crates/fakecloud-iotwireless/scripts, so the repo root is
# three directories up.
REPO = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))

# ---- metadata builder ----
import json
M=os.path.join(REPO,"aws-models","iotwireless.json")
d=json.load(open(M)); shapes=d['shapes']
NS="com.amazonaws.iotwireless#"
...
for k in ops:
    ...
    # Reclassify to a control-plane action/singleton in two cases:
    #  * a trailing FIXED segment (a sub-resource verb like
    #    `/wireless-gateways/{Id}/tasks` or `/wireless-devices/{Id}/data`), or
    #  * a get/update/delete with NO label (a singleton/collection endpoint).
    # Unlike AWS IoT Core, IoT Wireless models Create as a collection POST with
    # no path label (e.g. `POST /destinations`, `POST /device-profiles`) and the
    # new resource's identifier (a minted `Id`, or the body `Name`) is not in
    # the URI — so a label-less Create must stay a `create`, not become an
    # `action`. Only a trailing fixed segment demotes a Create.
    if verb == 'create':
        # A trailing fixed segment demotes a Create only when it follows a path
        # label (a sub-resource verb like `/wireless-gateways/{Id}/tasks`). A
        # bare collection POST (`/destinations`) is all-fixed but is a real
        # create — keep it.
        if last_fixed and label_segs:
            verb = 'action'
    elif verb in ('get','update','delete'):
...
json.dump(meta,open('iotwireless_meta.json','w'))
print("ops:",len(meta))
# stats
...
out=[]
out.append("// @generated by scripts/generate-iotwireless-tables.py (do not edit by hand)")
out.append("// AWS IoT Wireless (iotwireless) operation metadata derived from the Smithy model.")
...
out.append("];")
out.append("")
open(os.path.join(REPO,"crates","fakecloud-iotwireless","src","generated.rs"),"w").write("\n".join(out)+"\n")
print("wrote crates/fakecloud-iotwireless/src/generated.rs")
