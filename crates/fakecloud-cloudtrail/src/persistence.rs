...
use crate::state::{CloudTrailSnapshot, SharedCloudTrailState, CLOUDTRAIL_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read cloudtrail persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse cloudtrail persistence snapshot: {0}")]
    Parse(String),
    #[error("cloudtrail persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    let snapshot: CloudTrailSnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > CLOUDTRAIL_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: CLOUDTRAIL_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = CloudTrailSnapshot {
        schema_version: CLOUDTRAIL_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write cloudtrail snapshot"),
        Err(err) => tracing::error!(%err, "cloudtrail snapshot task panicked"),
    }
}
...
mod tests {
    use super::*;
    use crate::state::{CloudTrailData, CloudTrailSnapshot};
    ...
    fn state() -> SharedCloudTrailState {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<CloudTrailData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": CLOUDTRAIL_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
