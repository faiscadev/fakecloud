//! Model-driven input validation for AWS Shield.
//!
...
//! any business logic runs. This module reproduces that: the Shield Smithy
...
//! message for the first violation (surfaced as `InvalidParameterException`,
//! Shield's canonical request-validation error shape).
...
fn build_constraints() -> OpConstraints {
    let model: Value = serde_json::from_str(MODEL_JSON).expect("embedded shield model parses");
    let shapes = model
        .get("shapes")
...
mod tests {
    ...
    fn missing_required_is_rejected() {
        // CreateProtection requires Name + ResourceArn.
        let err =
            validate_input("CreateProtection", &json!({"ResourceArn": "arn:aws:x"})).unwrap_err();
    ...
    fn valid_input_passes() {
        assert!(validate_input(
            "CreateProtection",
            &json!({"Name": "web", "ResourceArn": "arn:aws:cloudfront::123456789012:distribution/E1"})
        )
        .is_ok());
    }
    ...
    fn enum_is_validated() {
        // CreateProtectionGroup.Aggregation is an enum (SUM | MEAN | MAX).
        let err = validate_input(
            "CreateProtectionGroup",
            &json!({
                "ProtectionGroupId": "grp", "Aggregation": "__INVALID__", "Pattern": "ALL"
            }),
        )
        .unwrap_err();
    ...
    fn length_is_validated() {
        // ProtectionGroupId max length is 36.
        let err = validate_input(
            "CreateProtectionGroup",
            &json!({
                "ProtectionGroupId": "x".repeat(37), "Aggregation": "SUM", "Pattern": "ALL"
            }),
        )
        .unwrap_err();
