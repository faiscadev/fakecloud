...
use crate::state::{SharedShieldState, ShieldSnapshot, SHIELD_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read shield persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse shield persistence snapshot: {0}")]
    Parse(String),
    #[error("shield persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    if snapshot.schema_version > SHIELD_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: SHIELD_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = ShieldSnapshot {
        schema_version: SHIELD_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write shield snapshot"),
        Err(err) => tracing::error!(%err, "shield snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<ShieldState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": SHIELD_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
