//! Snapshot save/load for DMS state.
...
use crate::state::{DmsSnapshot, SharedDmsState, DMS_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read dms persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse dms persistence snapshot: {0}")]
    Parse(String),
    #[error("dms persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub async fn save_snapshot(
    ...
    let snapshot = DmsSnapshot {
        schema_version: DMS_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write dms snapshot"),
        Err(err) => tracing::error!(%err, "dms snapshot task panicked"),
    }
}
...
mod tests {
    use super::*;
    use crate::state::{DmsData, DmsSnapshot};
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<DmsData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": DMS_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
