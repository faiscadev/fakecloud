//! Snapshot save/load for OpenSearch/Elasticsearch state.
...
use crate::state::{OpenSearchSnapshot, SharedOpenSearchState, OPENSEARCH_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read opensearch persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse opensearch persistence snapshot: {0}")]
    Parse(String),
    #[error("opensearch persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    let snapshot: OpenSearchSnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > OPENSEARCH_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: OPENSEARCH_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = OpenSearchSnapshot {
        schema_version: OPENSEARCH_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write opensearch snapshot"),
        Err(err) => tracing::error!(%err, "opensearch snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn state() -> SharedOpenSearchState {
    ...
    fn round_trip_restores_accounts() {
        let mut accounts: MultiAccountState<OpenSearchState> =
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<OpenSearchState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": OPENSEARCH_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
