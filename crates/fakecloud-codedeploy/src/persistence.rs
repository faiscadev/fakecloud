...
use crate::state::{CodeDeploySnapshot, SharedCodeDeployState, CODEDEPLOY_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read codedeploy persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse codedeploy persistence snapshot: {0}")]
    Parse(String),
    #[error("codedeploy persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub fn load_into(
    ...
    let snapshot: CodeDeploySnapshot =
        serde_json::from_slice(&bytes).map_err(|e| LoadError::Parse(e.to_string()))?;
    if snapshot.schema_version > CODEDEPLOY_SNAPSHOT_SCHEMA_VERSION {
        return Err(LoadError::SchemaTooNew {
            on_disk: snapshot.schema_version,
            supported: CODEDEPLOY_SNAPSHOT_SCHEMA_VERSION,
        });
    }
...
pub async fn save_snapshot(
    ...
    let snapshot = CodeDeploySnapshot {
        schema_version: CODEDEPLOY_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write codedeploy snapshot"),
        Err(err) => tracing::error!(%err, "codedeploy snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn state() -> SharedCodeDeployState {
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<CodeDeployState> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": CODEDEPLOY_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
