//! Parsing and evaluation of the `Filters` request member on the
//! RDS-family Query APIs.
//!
//! RDS, DocumentDB and Neptune share one shape here -- all three are
//! Query-protocol services whose Describe* operations take a
//! `FilterList`, and all three name their resources with `arn:aws:rds:`
//! ARNs, so the identifier helpers below apply unchanged to each.
//!
//! Their Describe* operations take a `FilterList` whose Smithy shape names
...
pub fn warn_unknown_filters(filters: &[QueryFilter], supported: &[&str]) {
...
pub fn sibling_rds_arn(arn: &str, resource_type: &str, id: &str) -> Option<String> {
...
pub fn normalized_identifier(param: Option<String>, resource_type: &str) -> Option<String> {
    param
...
pub fn optional_flag(value: Option<&str>) -> Option<bool> {
    match value?.trim() {
...
pub fn identifier_matches_type(param: &str, resource_type: &str) -> bool {
...
pub fn identifier_account(param: &str) -> Option<String> {
    if !param.starts_with("arn:") {
        return None;
    }
    param
...
pub fn addresses_own_account(param: &str, account_id: &str) -> bool {
...
pub fn parse_filters(req: &AwsRequest) -> Vec<QueryFilter> {
    ...
    let body = if filters_in_body {
        crate::protocol::parse_query_body(&req.body)
    } else {
        HashMap::new()
    };
    ...
    for index in 1.. {
        ...
        filters.push(QueryFilter { name, values });
    }
