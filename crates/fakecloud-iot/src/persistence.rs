...
use crate::state::{IotSnapshot, SharedIotState, IOT_SNAPSHOT_SCHEMA_VERSION};
...
pub enum LoadError {
    #[error("failed to read iot persistence snapshot: {0}")]
    Io(String),
    #[error("failed to parse iot persistence snapshot: {0}")]
    Parse(String),
    #[error("iot persistence schema too new: on-disk={on_disk}, max supported={supported}")]
...
pub async fn save_snapshot(
    ...
    let snapshot = IotSnapshot {
        schema_version: IOT_SNAPSHOT_SCHEMA_VERSION,
    ...
    match join {
        Ok(Ok(())) => {}
        Ok(Err(err)) => tracing::error!(%err, "failed to write iot snapshot"),
        Err(err) => tracing::error!(%err, "iot snapshot task panicked"),
    }
}
...
mod tests {
    ...
    fn round_trip_restores_resources() {
        let mut accounts: MultiAccountState<IotData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let data = accounts.get_or_create("111122223333");
        data.put_resource("things", "sensor-1", json!({"thingName": "sensor-1"}));
        let snap = IotSnapshot {
            schema_version: IOT_SNAPSHOT_SCHEMA_VERSION,
            accounts,
        };
        ...
        assert_eq!(
            acct.get_resource("things", "sensor-1").unwrap()["thingName"],
            "sensor-1"
        );
    }
    ...
    fn rejects_future_schema() {
        let accounts: MultiAccountState<IotData> =
            MultiAccountState::new("000000000000", "us-east-1", "");
        let bytes = serde_json::to_vec(&serde_json::json!({
            "schema_version": IOT_SNAPSHOT_SCHEMA_VERSION + 1,
            "accounts": accounts,
        }))
        .unwrap();
